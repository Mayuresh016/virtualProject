...
@Api(description = ".", tags = { "Druglicense Master APIs" })
@RequestMapping("/rest")
public class DruglicenseMasterController extends CrudService<DruglicensemasterBody> {

	private @Autowired DruglicenseMasterEntity druglicenseMasterEntity;
	...
	@PostMapping(path = "/druglicense_master", consumes = "application/json")
	...
	public String crudReq(@RequestBody @Validated DruglicensemasterBody body, HttpServletRequest request) {
		hRequest = request;
		String response;
	...
	@Override
	public boolean preCrudValidate(DruglicensemasterBody body) {
		errormsg = new HashMap<>();
	...
	@Override
	public String crudImpl(DruglicensemasterBody body) {
		try {
			return druglicenseMasterEntity.crud(hRequest, body);
	...
	@ResponseStatus(code = HttpStatus.OK)
	@GetMapping(path = "/druglicense_master/{DRUGLICENSEMASTERID}", produces = "application/json")
	@ApiOperation(value = "Retrieve a Druglicensemaster entity with a specified 'DRUGLICENSEMASTERID' column value", httpMethod = "GET")
	@ApiResponse(code = 200, message = "Returns a 200 response code if a Druglicensemaster entity has been retrieved successfully")
	public String get(@PathVariable(name = "DRUGLICENSEMASTERID") long druglicensemasterid, HttpServletRequest request) {
		return druglicenseMasterEntity.get(request, druglicensemasterid);
	}
