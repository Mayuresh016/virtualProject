...
import com.micropro.common.pharmazip.entity.MstAreaEntity;
...
@Api(description = ".", tags = { "Mst Area APIs" })
@RequestMapping("/rest")
public class MstAreaController extends CrudService<AreaBody> {

	private @Autowired MstAreaEntity mstAreaEntity;
	...
	@PostMapping(path = "/mst_area", consumes = "application/json")
	@ApiOperation(value = "CRUD operation on Area entity", httpMethod = "POST")
	@ApiResponse(code = 200, message = "Returns a 200 response code if a Area entity has been created successfully")
	public String crudReq(@RequestBody @Validated AreaBody body, HttpServletRequest request) {
		hRequest = request;
		String response;
	...
	@Override
	public boolean preCrudValidate(AreaBody body) {
		errormsg = new HashMap<>();
	...
	@Override
	public String crudImpl(AreaBody body) {
		try {
			return mstAreaEntity.crud(hRequest, body);
	...
	@ResponseStatus(code = HttpStatus.OK)
	@GetMapping(path = "/mst_area/{AREAID}", produces = "application/json")
	@ApiOperation(value = "Retrieve a Area entity with a specified 'AREAID' column value", httpMethod = "GET")
	@ApiResponse(code = 200, message = "Returns a 200 response code if a Area entity has been retrieved successfully")
	public String get(@PathVariable(name = "AREAID") long areaid, HttpServletRequest request) {
		return mstAreaEntity.get(request, areaid);
	}
