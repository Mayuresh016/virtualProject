...
import com.micropro.common.pharmazip.model.generated.MstBatchModel.BatchBody;
...
@Api(description = ".", tags = { "Mst Batch APIs" })
@RequestMapping("/rest")
public class MstBatchController extends CrudService<BatchBody> {

	private @Autowired MstBatchEntity mstBatchEntity;
	...
	@PostMapping(path = "/mst_batch", consumes = "application/json")
	...
	public String crudReq(@RequestBody @Validated BatchBody body, HttpServletRequest request) {
		hRequest = request;
		String response;
	...
	@Override
	public boolean preCrudValidate(BatchBody body) {
		errormsg = new HashMap<>();
	...
	@Override
	public String crudImpl(BatchBody body) {
		try {
			return mstBatchEntity.crud(hRequest, body);
	...
	@ResponseStatus(code = HttpStatus.OK)
	@GetMapping(path = "/mst_batch/{BATCHID}", produces = "application/json")
	@ApiOperation(value = "Retrieve a Batch entity with a specified 'BATCHID' column value", httpMethod = "GET")
	@ApiResponse(code = 200, message = "Returns a 200 response code if a Batch entity has been retrieved successfully")
	public String get(@PathVariable(name = "BATCHID") long batchid, HttpServletRequest request) {
		return mstBatchEntity.get(request, batchid);
	}
