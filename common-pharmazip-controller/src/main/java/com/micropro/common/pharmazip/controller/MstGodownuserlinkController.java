...
@Api(description = ".", tags = { "Mst Godownuserlink APIs" })
@RequestMapping("/rest")
public class MstGodownuserlinkController extends CrudService<GodownuserlinkBody> {

	private @Autowired MstGodownuserlinkEntity mstGodownuserlinkEntity;
	...
	@PostMapping(path = "/mst_godownuserlink", consumes = "application/json")
	...
	public String crudReq(@RequestBody @Validated GodownuserlinkBody body, HttpServletRequest request) {
		hRequest = request;
		String response;
	...
	@Override
	public boolean preCrudValidate(GodownuserlinkBody body) {
		errormsg = new HashMap<>();
	...
	@Override
	public String crudImpl(GodownuserlinkBody body) {
		try {
			return mstGodownuserlinkEntity.crud(hRequest, body);
	...
	@ResponseStatus(code = HttpStatus.OK)
	@GetMapping(path = "/mst_godownuserlink/{GODOWNUSERLINKID}", produces = "application/json")
	@ApiOperation(value = "Retrieve a Godownuserlink entity with a specified 'GODOWNUSERLINKID' column value", httpMethod = "GET")
	@ApiResponse(code = 200, message = "Returns a 200 response code if a Godownuserlink entity has been retrieved successfully")
	public String get(@PathVariable(name = "GODOWNUSERLINKID") long godownuserlinkid, HttpServletRequest request) {
		return mstGodownuserlinkEntity.get(request, godownuserlinkid);
	}
