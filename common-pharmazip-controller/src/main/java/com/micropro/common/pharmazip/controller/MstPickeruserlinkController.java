...
@Api(description = ".", tags = { "Mst Pickeruserlink APIs" })
@RequestMapping("/rest")
public class MstPickeruserlinkController extends CrudService<PickeruserlinkBody> {

	private @Autowired MstPickeruserlinkEntity mstPickeruserlinkEntity;
	...
	@PostMapping(path = "/mst_pickeruserlink", consumes = "application/json")
	...
	public String crudReq(@RequestBody @Validated PickeruserlinkBody body, HttpServletRequest request) {
		hRequest = request;
		String response;
	...
	@Override
	public boolean preCrudValidate(PickeruserlinkBody body) {
		errormsg = new HashMap<>();
	...
	@Override
	public String crudImpl(PickeruserlinkBody body) {
		try {
			return mstPickeruserlinkEntity.crud(hRequest, body);
	...
	@ResponseStatus(code = HttpStatus.OK)
	@GetMapping(path = "/mst_pickeruserlink/{PICKERUSERLINKID}", produces = "application/json")
	@ApiOperation(value = "Retrieve a Pickeruserlink entity with a specified 'PICKERUSERLINKID' column value", httpMethod = "GET")
	@ApiResponse(code = 200, message = "Returns a 200 response code if a Pickeruserlink entity has been retrieved successfully")
	public String get(@PathVariable(name = "PICKERUSERLINKID") long pickeruserlinkid, HttpServletRequest request) {
		return mstPickeruserlinkEntity.get(request, pickeruserlinkid);
	}
