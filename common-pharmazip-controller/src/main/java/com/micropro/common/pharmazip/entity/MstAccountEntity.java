...
public class MstAccountEntity {
	...
	public String crud(HttpServletRequest request, MstAccountBody body) throws JsonProcessingException {
	...
	public String get(HttpServletRequest request, long id) {
		...
		if(data.size() > 0) {
			JsonObject account = (JsonObject) data.get(0);

			// Map Entity 1
			query = "SELECT * FROM accountoprdetail where accountid = " + id;
			data = utility.executeQueryOnPool(query, request);

			account.add("accountoprdetail", data);
