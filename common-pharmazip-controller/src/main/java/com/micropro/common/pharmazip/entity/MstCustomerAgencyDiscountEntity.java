...
@Component
public class MstCustomerAgencyDiscountEntity {

	private @Autowired ConnectionUtility utility;
	private @Autowired CommonDateFormater formater;

	public String crud(HttpServletRequest request, CustomeragencydiscountBody body) throws JsonProcessingException {
		String token = request.getHeader("authorization").split(" ")[1];
		String rightId = request.getHeader("rightId");

		JsonObject obj = formater.serializeObject(body);
		String reqBody = "{\"customeragencydiscount\":"  + obj.toString() + "}";
	...
	public String get(HttpServletRequest request, long id) {
		...
		// Root Entity
		String query = "SELECT * FROM customeragencydiscount where customeragencydiscountid = " + id;
