...
@Component
public class MstPickerEntity {

	private @Autowired ConnectionUtility utility;
	private @Autowired CommonDateFormater formater;

	public String crud(HttpServletRequest request, MstPickerBody body) throws JsonProcessingException {
	...
	public String get(HttpServletRequest request, long id) {
		...
		// Root Entity
		String query = "SELECT * FROM picker where pickerid = " + id;

		JsonArray data = utility.executeQueryOnPool(query, request);
		if(data.size() > 0) {
			JsonObject picker = (JsonObject) data.get(0);

			// Map Entity 1
			query = "SELECT * FROM pickerlocationlink where pickerid = " + id;
			data = utility.executeQueryOnPool(query, request);

			picker.add("pickerlocationlink", data);

			response.add("picker", picker);
		}
