...
public class MstProductEntity {
	...
	public String crud(HttpServletRequest request, MstProductBody body) throws JsonProcessingException {
	...
	public String get(HttpServletRequest request, long id) {
		...
		if(data.size() > 0) {
			JsonObject product = (JsonObject) data.get(0);

			// Map Entity 1
			query = "SELECT * FROM productoprdtl where productid = " + id;
			data = utility.executeQueryOnPool(query, request);

			product.add("productoprdtl", data);
