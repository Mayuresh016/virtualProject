...
@Component
public class MstStoreEntity {

	private @Autowired ConnectionUtility utility;
	private @Autowired CommonDateFormater formater;

	public String crud(HttpServletRequest request, MstStoreBody body) throws JsonProcessingException {
	...
	public String get(HttpServletRequest request, long id) {
		...
		if(data.size() > 0) {
			JsonObject store = (JsonObject) data.get(0);

			// Map Entity 1
			query = "SELECT * FROM location where storeid = " + id;
			data = utility.executeQueryOnPool(query, request);

			store.add("location", data);

			response.add("store", store);
		}
