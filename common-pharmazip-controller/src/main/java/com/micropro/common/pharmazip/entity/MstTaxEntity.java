...
public class MstTaxEntity {
	...
	public String crud(HttpServletRequest request, MstTaxBody body) throws JsonProcessingException {
	...
	public String get(HttpServletRequest request, long id) {
		...
		if(data.size() > 0) {
			JsonObject tax = (JsonObject) data.get(0);

			// Map Entity 1
			query = "SELECT * FROM taxaccount where taxid = " + id;
			data = utility.executeQueryOnPool(query, request);

			tax.add("taxaccount", data);

			response.add("tax", tax);
		}
