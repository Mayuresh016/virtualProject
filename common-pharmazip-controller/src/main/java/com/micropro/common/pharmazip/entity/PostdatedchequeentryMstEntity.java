...
@Component
public class PostdatedchequeentryMstEntity {

	private @Autowired ConnectionUtility utility;
	private @Autowired CommonDateFormater formater;

	public String crud(HttpServletRequest request, PostdatedchequeentryBody body) throws JsonProcessingException {
		String token = request.getHeader("authorization").split(" ")[1];
		String rightId = request.getHeader("rightId");

		JsonObject obj = formater.serializeObject(body);
		String reqBody = "{\"postdatedchequeentry\":"  + obj.toString() + "}";
	...
	public String get(HttpServletRequest request, long id) {
		...
		// Root Entity
		String query = "SELECT * FROM postdatedchequeentry where postdatedchequeentryid = " + id;
