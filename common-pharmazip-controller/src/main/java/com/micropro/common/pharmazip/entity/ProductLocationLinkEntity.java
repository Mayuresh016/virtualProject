...
@Component
public class ProductLocationLinkEntity {

	private @Autowired ConnectionUtility utility;
	private @Autowired CommonDateFormater formater;

	public String crud(HttpServletRequest request, ProductlocationlinkBody body) throws JsonProcessingException {
		String token = request.getHeader("authorization").split(" ")[1];
		String rightId = request.getHeader("rightId");

		JsonObject obj = formater.serializeObject(body);
		String reqBody = "{\"productlocationlink\":"  + obj.toString() + "}";
	...
	public String get(HttpServletRequest request, long id) {
		...
		// Root Entity
		String query = "SELECT * FROM productlocationlink where productlocationlinkid = " + id;
