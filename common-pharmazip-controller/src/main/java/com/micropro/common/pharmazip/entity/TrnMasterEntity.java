...
@Component
public class TrnMasterEntity {

	private @Autowired ConnectionUtility utility;
	private @Autowired CommonDateFormater formater;

	public String crud(HttpServletRequest request, TrnMasterBody body) throws JsonProcessingException {
	...
	public String get(HttpServletRequest request, long id) {
		...
		// Root Entity
		String query = "SELECT * FROM transactionmaster where transactionmasterid = " + id;

		JsonArray data = utility.executeQueryOnPool(query, request);
		if(data.size() > 0) {
			JsonObject transactionmaster = (JsonObject) data.get(0);

			// Map Entity 1
			query = "SELECT * FROM transactiondetail where transactionmasterid = " + id;
			data = utility.executeQueryOnPool(query, request);

			transactionmaster.add("transactiondetail", data);

			response.add("transactionmaster", transactionmaster);
		}
